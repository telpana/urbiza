...
export default function Home() {
    ...
    const cargar = async () => {
      ...
      const [{ data: sd }, { data: pc }, { data: stg }] = await Promise.all([
        ...
        supabase.from('propiedades').select(campos).eq('estado', 'activo')
          ...
          .order('created_at', { ascending: false }).limit(4),
        supabase.from('propiedades').select(campos).eq('estado', 'activo')
          .or('zona.ilike.%Samaná%,zona.ilike.%Las Terrenas%,zona.ilike.%Las Galeras%,zona.ilike.%El Portillo%,zona.ilike.%Cosón%,zona.ilike.%El Limón%,zona.ilike.%Santa Bárbara%')
          .order('created_at', { ascending: false }).limit(4),
      ])
  ...
  return (
    <main style={{ fontFamily: 'sans-serif', margin: 0, padding: 0, background: '#f4f5f6' }}>
      ...
      {/* BANNER PROFESIONAL */}
      <div className="pro-banner-outer" style={{ position: 'relative', height: 200, borderTop: '1px solid #e8e8e8', borderBottom: '1px solid #e8e8e8', overflow: 'hidden', background: '#e8f0f0' }}>
        {proImgUrl && <Image src={proImgUrl} alt="" fill sizes="100vw" style={{ objectFit: 'cover', objectPosition: 'left center' }} />}
        <div className="pro-banner-card" style={{ position: 'absolute', right: 80, top: '50%', transform: 'translateY(-50%)', background: '#fff', padding: '24px 28px', width: 300, boxShadow: '0 4px 24px rgba(0,0,0,0.1)' }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#111', lineHeight: 1.3, marginBottom: 8 }}>¿Eres agente o promotor inmobiliario?</div>
          <div style={{ fontSize: 13, color: '#555', lineHeight: 1.5, marginBottom: 14 }}>Llega a miles de compradores. Sin permanencia.</div>
          <a href="/registro" style={{ color: '#006D77', fontWeight: 600, fontSize: 13, textDecoration: 'none' }}>Publicar por US$9.99/mes →</a>
        </div>
      </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              {instagramUrl && (
                <a href={instagramUrl} target="_blank" rel="noopener noreferrer" aria-label="Instagram" style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' }}>
              ...
              {facebookUrl && (
                <a href={facebookUrl} target="_blank" rel="noopener noreferrer" aria-label="Facebook" style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' }}>
              ...
              {tiktokUrl && (
                <a href={tiktokUrl} target="_blank" rel="noopener noreferrer" aria-label="TikTok" style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' }}>
