...
export default function Registro() {
  ...
  const tipos = [
    ...
    { id: 'broker', titulo: 'Broker', desc: 'Profesional independiente con anuncios ilimitados', icon: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#006D77" strokeWidth="1.5"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><line x1="12" y1="12" x2="12" y2="16"/><line x1="10" y1="14" x2="14" y2="14"/></svg> },
    { id: 'agencia', titulo: 'Agencia', desc: 'Inmobiliaria con equipo de brokers y panel de gestión', icon: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#006D77" strokeWidth="1.5"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> },
  ]

  const totalPasos = metodo === 'email' ? 4 : 3
        ...
        <div style={{ width: '100%', maxWidth: 480 }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 14, color: '#888' }}>
              ...
              {paso === 3 && metodo === 'email' && 'Tus datos'}
              {paso === 3 && metodo !== 'email' && 'Casi listo'}
              {paso === 4 && '¡Cuenta creada!'}
            </div>
          </div>

          {/* INDICADOR DE PASOS */}
          {paso > 1 && paso < totalPasos + 1 && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0, marginBottom: 24 }}>
              {Array.from({ length: totalPasos }, (_, i) => i + 1).map((n, i) => (
                <div key={n} style={{ display: 'flex', alignItems: 'center' }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: paso >= n + 1 ? '#006D77' : '#e0e0e0', color: paso >= n + 1 ? '#fff' : '#aaa', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700 }}>{n}</div>
                  {i < totalPasos - 1 && <div style={{ width: 50, height: 2, background: paso > n + 1 ? '#006D77' : '#e0e0e0' }} />}
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ background: '#fff', borderRadius: 10, padding: '28px', boxShadow: '0 2px 16px rgba(0,0,0,0.08)' }}>
            ...
            {paso === 1 && (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
                  <button onClick={() => { setMetodo('google'); setPaso(2) }} style={{ all: 'unset', width: '100%', border: '1.5px solid #e0e0e0', borderRadius: 7, padding: '13px', fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, boxSizing: 'border-box', color: '#333' }}
                  ...
                  <button onClick={() => { setMetodo('facebook'); setPaso(2) }} style={{ all: 'unset', width: '100%', border: '1.5px solid #e0e0e0', borderRadius: 7, padding: '13px', fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, boxSizing: 'border-box', color: '#333' }}
            ...
            {paso === 2 && (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
                  {tipos.map(t => (
                    <div key={t.id} onClick={() => setTipoCuenta(t.id)} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 18px', border: `2px solid ${tipoCuenta === t.id ? '#006D77' : '#e0e0e0'}`, borderRadius: 8, cursor: 'pointer', background: tipoCuenta === t.id ? '#f0fafb' : '#fff', transition: 'all 0.15s' }}>
            ...
            {/* PASO 3 — DATOS (solo si eligió email) */}
            {paso === 3 && metodo === 'email' && (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {(tipoCuenta === 'broker' || tipoCuenta === 'agencia') && (
                    <div>
                  ...
                  {(tipoCuenta === 'broker' || tipoCuenta === 'agencia') && (
                    <div>
                      <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 6 }}>
                        Número AEI <span style={{ fontSize: 11, color: '#aaa', fontWeight: 400 }}>(opcional)</span>
                      </label>
                      ...
                      <div style={{ fontSize: 11, color: '#aaa', marginTop: 4 }}>Si introduces tu número AEI lo verificamos en 24-48h y aparece el badge en tus anuncios</div>
                    </div>
                  )}
                  <div>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 6 }}>Contraseña</label>
                    <input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="Mínimo 8 caracteres" style={{ width: '100%', border: '1.5px solid #e0e0e0', borderRadius: 6, padding: '11px 14px', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} onFocus={e => e.target.style.borderColor='#006D77'} onBlur={e => e.target.style.borderColor='#e0e0e0'} />
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                  ...
                  <button onClick={() => setPaso(4)} style={{ all: 'unset', flex: 2, background: '#006D77', color: '#fff', padding: '12px', borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: 'pointer', textAlign: 'center', boxSizing: 'border-box' }}>Crear cuenta</button>
                </div>
              </div>
            )}
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ fontSize: 14, color: '#777', marginBottom: 6, lineHeight: 1.6 }}>
                  {metodo === 'email'
                    ...
                    : <>Tu cuenta ha sido creada con {metodo === 'google' ? 'Google' : 'Facebook'} correctamente</>
                  }
                </p>
                {aei && (
                  <div style={{ background: '#e0f5f7', border: '1px solid #c5e8ea', borderRadius: 6, padding: '10px 14px', fontSize: 13, color: '#004E57', margin: '16px 0' }}>
