...
export async function generateMetadata(
  ...
  if (!meta) return { title: 'Propiedades en alquiler | Habitade' }

  const title = `Propiedades en alquiler en ${meta.nombre}, República Dominicana`
  const description = `Alquila apartamentos, casas y villas en ${meta.nombre}. Encuentra las mejores propiedades en arrendamiento en ${meta.nombre}, República Dominicana en Habitade.`
  const url = `https://www.habitade.com/alquiler/${zona}`
...
export default async function AlquilerZonaPage({ params }: { params: Promise<{ zona: string }> }) {
  const { zona } = await params
  const meta = zonasMeta[zona]
  if (!meta) notFound()
  ...
  const jsonLd = {
    ...
    '@type': 'ItemList',
    name: `Propiedades en alquiler en ${meta.nombre}`,
    ...
    <>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 20px', fontFamily: 'sans-serif' }}>
        <nav style={{ fontSize: 13, color: '#888', marginBottom: 20 }}>
          ...
          {' › '}
          <a href="/buscar?operacion=Alquiler" style={{ color: '#006D77', textDecoration: 'none' }}>Alquilar</a>
          {' › '}
          <span>{meta.nombre}</span>
        </nav>

        <h1 style={{ fontSize: 28, fontWeight: 700, color: '#111', marginBottom: 8 }}>
          Propiedades en alquiler en {meta.nombre}
        </h1>
        ...
        {lista.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 0', color: '#aaa' }}>
            ...
            <a href="/buscar?operacion=Alquiler" style={{ color: '#006D77', fontSize: 14 }}>Ver todas las propiedades en alquiler →</a>
          </div>
        ) : (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
              {lista.map((p: any) => {
                ...
                return (
                  <a key={p.id} href={`/propiedad/${p.id}`} style={{ textDecoration: 'none', borderRadius: 10, overflow: 'hidden', border: '1px solid #eee', display: 'block' }}>
                    ...
                    <div style={{ padding: '14px 16px' }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 700, color: '#006D77', marginBottom: 6 }}>{fmtPrecio(p.precio)}<span style={{ fontSize: 12, fontWeight: 400, color: '#888' }}>/mes</span></div>
            ...
            <div style={{ marginTop: 40, textAlign: 'center' }}>
              <a href={`/buscar?operacion=Alquiler&zona=${encodeURIComponent(nombreZona)}`} style={{ display: 'inline-block', background: '#006D77', color: '#fff', padding: '12px 32px', borderRadius: 8, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
