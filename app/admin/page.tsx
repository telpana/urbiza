...
const I = (paths: React.ReactNode) => <svg width="18" height="18" viewBox="2 2 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" strokeLinecap="round" style={{ overflow: 'visible', flexShrink: 0 }}>{paths}</svg>

const menuItems = [
  { id: 'dashboard', label: 'Dashboard', icon: I(<><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></>) },
  { id: 'aei', label: 'Verificaciones AEI', icon: I(<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>) },
  { id: 'propiedades', label: 'Anuncios', icon: I(<><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></>) },
  { id: 'usuarios', label: 'Usuarios', icon: I(<><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></>) },
  { id: 'cobros', label: 'Suscripciones', icon: I(<><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></>) },
  { id: 'destacados', label: 'Destacados', icon: I(<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>) },
  { id: 'banner', label: 'Banner inicio', icon: I(<><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></>) },
  { id: 'favicon', label: 'Favicon e img', icon: I(<><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></>) },
  { id: 'redes', label: 'Redes sociales', icon: I(<><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></>) },
  { id: 'codigos', label: 'Códigos promo', icon: I(<><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></>) },
  { id: 'rankings', label: 'Estadísticas', icon: I(<><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></>) },
  { id: 'reportes', label: 'Reportes', icon: I(<><path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/><polyline points="13 2 13 9 20 9"/><line x1="12" y1="13" x2="12" y2="17"/><line x1="12" y1="11" x2="12.01" y2="11"/></>) },
  { id: 'ayuda', label: 'Ayuda usuarios', icon: I(<><circle cx="12" cy="12" r="9"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></>) },
]
...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    if (seccion === 'rankings') {
      ...
      fetch('/api/admin/ranking', { headers: authHeader() })
        .then(r => r.json()).then(d => { setRankings(d); setRankingsLoading(false) })
    }
    if (seccion === 'reportes') {
    ...
    <main style={{ fontFamily: 'sans-serif', margin: 0, padding: 0, background: C.bg, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        <div style={{ marginLeft: isMobile ? 0 : 220, padding: isMobile ? '16px' : '28px 32px', minHeight: 'calc(100vh - 54px)', minWidth: 0 }}>
            ...
            <Seccion titulo="Dashboard">
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: 16, marginBottom: 28 }}>
                {(() => {
                  ...
                  return [
                    { label: 'Visitas web:', val: fmt(stats?.totalVisitas), color: '#ef4444', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="1.8"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg> },
                    { label: 'Anuncios activos:', val: fmt(stats?.totalProp), color: C.verde, icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={C.verde} strokeWidth="1.8"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg> },
                    { label: 'Usuarios registrados:', val: fmt(stats?.totalUsers), color: C.azul, icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={C.azul} strokeWidth="1.8"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg> },
                    { label: 'Suscripciones activas:', val: fmt(stats?.totalSubs), color: '#10b981', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="1.8"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg> },
                    { label: 'Destacados activos:', val: fmt(stats?.totalDestacados), color: '#8b5cf6', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" strokeWidth="1.8"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg> },
                    { label: 'AEI pendientes:', val: fmt(stats?.aeiPend), color: '#f59e0b', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="1.8"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg> },
                  ].map(s => (
          ...
          {seccion === 'rankings' && (
            <Seccion titulo="Estadísticas de perfiles" desc="Rankings históricos acumulando todas las propiedades activas y pasadas de cada usuario">
              {rankingsLoading ? (
                <div style={{ color: '#888', fontSize: 14, padding: 40, textAlign: 'center' }}>Cargando rankings...</div>
              ) : rankings && (() => {
                const cols: { key: 'masVisitados'|'masCompartidos'|'masContactados'|'masGuardados'; titulo: string; metrica: string; campo: string; color: string; icono: React.ReactNode }[] = [
                  { key: 'masVisitados',   titulo: 'Perfil más visitado',    metrica: 'visitas totales',  campo: 'visitas',    color: '#006D77', icono: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg> },
                  { key: 'masCompartidos', titulo: 'Perfil más compartido',  metrica: 'veces compartido', campo: 'compartidos', color: '#8b5cf6', icono: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg> },
                  { key: 'masContactados', titulo: 'Perfil más contactado',  metrica: 'vistas de teléfono', campo: 'tel_vistos', color: '#10b981', icono: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 9.91a16 16 0 0 0 6.29 6.29l.91-.91a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg> },
                  { key: 'masGuardados',   titulo: 'Perfil más guardado',    metrica: 'propiedades guardadas', campo: 'count', color: '#f59e0b', icono: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg> },
                ]
                return (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
                    {cols.map(({ key, titulo, metrica, campo, color, icono }) => (
                      <Card key={key}>
                        <div style={{ padding: '16px 20px 12px', borderBottom: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ background: color, borderRadius: 6, padding: 6, display: 'flex', alignItems: 'center', color: '#fff' }}>{icono}</span>
                          <div>
                            <div style={{ fontSize: 14, fontWeight: 700, color: '#111' }}>{titulo}</div>
                            <div style={{ fontSize: 11, color: '#aaa' }}>{metrica}</div>
                          </div>
                        </div>
                        <div style={{ padding: '8px 0' }}>
                          {(rankings[key] as any[]).length === 0 ? (
                            <div style={{ padding: '20px', fontSize: 13, color: '#aaa', textAlign: 'center' }}>Sin datos</div>
                          ) : (rankings[key] as any[]).map((u: any, i: number) => (
                            <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 20px', borderBottom: i < rankings[key].length - 1 ? '1px solid #f7f7f7' : 'none' }}>
                              <span style={{ fontSize: 13, fontWeight: 700, color: i === 0 ? color : '#bbb', minWidth: 18, textAlign: 'center' }}>#{i + 1}</span>
                              <div style={{ width: 32, height: 32, borderRadius: '50%', background: color + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
                                {u.foto_url
                                  ? <img src={u.foto_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                  : <span style={{ fontSize: 13, fontWeight: 700, color }}>{(u.nombre || u.email || '?')[0].toUpperCase()}</span>
                                }
                              </div>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: 13, fontWeight: 600, color: '#111', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.nombre || '—'}</div>
                                <div style={{ fontSize: 11, color: '#aaa', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.email}</div>
                              </div>
                              <span style={{ fontSize: 14, fontWeight: 700, color, flexShrink: 0 }}>{(u[campo] || 0).toLocaleString()}</span>
                            </div>
                          ))}
                        </div>
                      </Card>
