  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#f4f5f6' }}>
        <div style={{ color: '#006D77', fontSize: 15 }}>{T.cargando}</div>
      </div>
    )
  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 54px)', padding: '40px 20px' }}>
        <div style={{ width: '100%', maxWidth: 460 }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: '#111', marginBottom: 6 }}>{T.titulo}</div>
            <div style={{ fontSize: 14, color: '#888' }}>{T.subtitulo}</div>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 10, padding: '32px 28px', boxShadow: '0 2px 16px rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 10 }}>{T.tipoCuenta}</label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  { id: 'particular', label: T.particular, desc: T.particularDesc },
                  { id: 'profesional', label: T.profesional, desc: T.profesionalDesc },
                ].map(t => (
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder={T.nombrePlaceholder}
            ...
            <div style={{ marginBottom: tipo === 'profesional' ? 16 : 24 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 6 }}>
                {T.cedula} <span style={{ fontWeight: 400, color: '#aaa' }}>{T.opcional}</span>
              </label>
            ...
            {tipo === 'profesional' && (
              <div style={{ marginBottom: 24 }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 6 }}>
                  {T.aei} <span style={{ fontWeight: 400, color: '#aaa' }}>{T.opcional}</span>
                </label>
            ...
            <button onClick={guardar} disabled={guardando} style={{ all: 'unset', width: '100%', background: guardando ? '#aaa' : '#006D77', color: '#fff', padding: '13px', borderRadius: 6, fontSize: 15, fontWeight: 600, cursor: guardando ? 'default' : 'pointer', textAlign: 'center', display: 'block', boxSizing: 'border-box' }}>
              {guardando ? T.guardando : T.entrar}
            </button>
