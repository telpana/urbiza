...
const gruposZona: Record<string, string[]> = {
  ...
  'distrito nacional':     ['Distrito Nacional', 'Piantini', 'Naco', 'Serrallés', 'Bella Vista', 'Arroyo Hondo', 'Viejo Arroyo Hondo', 'Evaristo Morales', 'Los Cacicazgos', 'Gazcue', 'Ciudad Colonial', 'Miramar', 'La Esperilla', 'Cristo Rey', 'Villa Consuelo', 'Ensanche Ozama', 'Ensanche Isabelita', 'Urbanización Real', 'Quisqueya', 'Mirador Norte', 'Mirador Sur', 'Altos de Arroyo Hondo', 'Los Prados', 'Fernández'],
  'santo domingo':         ['Santo Domingo', 'Distrito Nacional', 'Piantini', 'Naco', 'Serrallés', 'Bella Vista', 'Arroyo Hondo', 'Viejo Arroyo Hondo', 'Evaristo Morales', 'Los Cacicazgos', 'Gazcue', 'Ciudad Colonial', 'Miramar', 'La Esperilla', 'Cristo Rey', 'Villa Consuelo', 'Ensanche Ozama', 'Ensanche Isabelita', 'Urbanización Real', 'Santo Domingo Este', 'Santo Domingo Norte', 'Santo Domingo Oeste', 'Boca Chica', 'Los Mina', 'Los Tres Brazos', 'Alma Rosa', 'San Isidro'],
...
const PROVINCIAS_SET = new Set([
  'Punta Cana', 'La Altagracia', 'Bávaro', 'Distrito Nacional', 'Santo Domingo',
  'Santiago', 'Puerto Plata', 'Samaná', 'Las Terrenas', 'La Romana',
  'San Pedro de Macorís', 'La Vega', 'María Trinidad Sánchez', 'El Seibo',
  'Hato Mayor', 'San Cristóbal', 'Peravia', 'Espaillat', 'Duarte',
  'Monseñor Nouel', 'Valverde', 'Monte Cristi', 'Dajabón', 'Azua', 'Barahona', 'Pedernales',
])
...
function BuscarContent() {
  ...
  const handleQueryChange = (val: string) => {
    ...
    if (val.length >= 2) {
      ...
      const prov = zonasSugerencias.filter(z => PROVINCIAS_SET.has(z) && normalize(z).includes(nv)).slice(0, 3)
      const zonas = zonasSugerencias.filter(z => !PROVINCIAS_SET.has(z) && normalize(z).includes(nv)).slice(0, 5)
      setSugerencias([...prov, ...zonas])
          ...
          <div style={{ position: 'absolute', top: 54, left: 0, right: 0, background: '#fff', boxShadow: '0 12px 32px rgba(0,0,0,0.15)', borderRadius: '0 0 16px 16px', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            {authReady && sesionActiva ? (<>
              <div style={{ padding: '4px 0' }}>
                <div style={{ padding: '10px 20px 4px', fontSize: 10, fontWeight: 700, color: '#bbb', textTransform: 'uppercase', letterSpacing: 1 }}>{Tn.miCuenta ?? 'Mi cuenta'}</div>
                {[
              ...
              <div style={{ padding: '14px 16px', display: 'flex', gap: 10 }}>
                <a href="/login" style={{ flex: 1, display: 'block', textAlign: 'center', padding: '11px', fontSize: 14, fontWeight: 600, color: '#006D77', border: '1.5px solid #006D77', borderRadius: 8, textDecoration: 'none' }}>{Tn.entrar}</a>
                <a href="/registro" style={{ flex: 1, display: 'block', textAlign: 'center', padding: '11px', fontSize: 14, fontWeight: 600, color: '#fff', background: '#006D77', borderRadius: 8, textDecoration: 'none' }}>{Tn.publicar}</a>
              </div>
            ) : null}
              ...
              <div key={p.id} className={`prop-card${p.dest ? ' prop-card-dest' : ''}`} style={{ display: 'flex', background: p.dest ? '#f0fafb' : '#fff', borderBottom: '8px solid #f4f5f6', borderLeft: p.dest ? '4px solid #006D77' : 'none', cursor: 'pointer', boxShadow: p.dest ? '0 4px 18px rgba(0,109,119,0.12)' : 'none' }}
                ...
                <div className="prop-card-img prop-card-img-slider"
                  ...
                  {p.fotos && p.fotos.length > 0
                    ? <img src={p.fotos[fotoIdx[String(p.id)] ?? 0]} alt={p.titulo} loading="lazy" decoding="async" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', transition: 'opacity 0.2s' }} />
