...
const provinciasZonas: Record<string, string[]> = {
  ...
  'La Altagracia': ['Bávaro', 'Punta Cana', 'Downtown Punta Cana', 'Cap Cana', 'Cabeza de Toro', 'Los Corales', 'Uvero Alto', 'Macao', 'Cortecito', 'Higüey', 'San Rafael del Yuma', 'Bayahibe', 'Dominicus'],
  'Santiago': ['Los Jardines', 'Cerros de Gurabo', 'Bella Vista', 'Villa Olga', 'Pontezuela', 'Las Colinas'],
  ...
  'Peravia': ['Baní'],
  'Espaillat': ['Moca', 'Gaspar Hernández'],
...
export default function Panel() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      if (perfil) {
        ...
        setPerfilAei(perfil.numero_aei ? perfil.numero_aei.replace(/^(AEI[-\s]*)+/i, '').trim().replace(/\D/g, '') : '')
  ...
  const guardarPerfil = async () => {
    ...
    const updates: any = {
      ...
      numero_aei: perfilAei ? `AEI-${perfilAei.replace(/\D/g, '')}` : null,
              ...
              {(perfilNombre && perfilTelefono) && (usuario?.tipo === 'profesional' && usuario?.plan !== 'profesional' ? (
                <div style={{ background: '#fff', borderRadius: 8, padding: '48px 32px', boxShadow: '0 1px 6px rgba(0,0,0,0.06)', textAlign: 'center' }}>
                  ...
                  <div style={{ maxWidth: 320, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <button onClick={handleSuscribirse} disabled={promoLoading} style={{ background: promoLoading ? '#aaa' : '#006D77', color: '#fff', padding: '11px 24px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: promoLoading ? 'default' : 'pointer', border: 'none', width: '100%', animation: promoValidado ? 'promo-pulse 1.5s ease-in-out infinite' : undefined }}>
                ...
                {!['Edificio', 'Terreno'].includes(pubTipo) && (
                  <div className="pub-form-grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 16 }}>
                    ...
                    <div className="pub-planta-col">
                      <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 6 }}>{Tpanel.publicar.piso} <span style={{ color: '#aaa', fontWeight: 400 }}>{Tpanel.publicar.sectorOpcional}</span></label>
                      <input type="text" value={pubPlanta} onChange={e => setPubPlanta(e.target.value)} placeholder={Tpanel.publicar.pisoPlaceholder} style={{ width: '100%', border: '1.5px solid #e0e0e0', borderRadius: 6, padding: '10px 12px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} onFocus={e => e.target.style.borderColor='#006D77'} onBlur={e => e.target.style.borderColor='#e0e0e0'} />
                    </div>
                  </div>
                )}
          ...
          {!cargando && seccion === 'planes' && (
            <div style={{ maxWidth: 480, margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{ background: '#fff', borderRadius: 10, padding: '32px 28px', boxShadow: '0 2px 16px rgba(0,109,119,0.12)', border: '2px solid #006D77', position: 'relative' }}>
                ...
                <button onClick={handleSuscribirse} disabled={promoLoading} style={{ display: 'block', width: '100%', background: promoLoading ? '#aaa' : '#006D77', color: '#fff', padding: '14px', borderRadius: 6, fontSize: 15, fontWeight: 700, cursor: promoLoading ? 'default' : 'pointer', border: 'none', animation: promoValidado ? 'promo-pulse 1.5s ease-in-out infinite' : undefined }}>
                  {promoLoading ? 'Procesando...' : promoValidado
                    ? <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}><span>Activar</span><span style={{ fontSize: 11, opacity: 0.85, fontWeight: 500 }}>{promoDias} días gratis</span></div>
                    : Tpanel.planes.suscribirse}
                </button>
            ...
            <div>
              ...
              {usuario?.plan !== 'profesional' ? (
                <div className="plan-upgrade-card" style={{ background: '#fff', borderRadius: 8, padding: '36px 28px', boxShadow: '0 1px 6px rgba(0,0,0,0.06)', textAlign: 'center' }}>
                  ...
                  <button className="plan-upgrade-cta" onClick={handleSuscribirse} disabled={promoLoading} style={{ background: promoLoading ? '#aaa' : '#006D77', color: '#fff', padding: '12px 32px', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: promoLoading ? 'default' : 'pointer', border: 'none', display: 'inline-block', animation: promoValidado ? 'promo-pulse 1.5s ease-in-out infinite' : undefined }}>
                    {promoLoading ? 'Procesando...' : promoValidado
                      ? <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}><span>Activar</span><span style={{ fontSize: 11, opacity: 0.85, fontWeight: 500 }}>{promoDias} días gratis</span></div>
                      : Tpanel.planes.suscribirse}
                  </button>
                ...
                <div className="perfil-fields-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  ...
                  {tipoUsuario === 'profesional' && usuario?.plan === 'profesional' && (
                    <div>
                      ...
                      <div data-aei style={{ display: 'flex', alignItems: 'center', border: `1.5px solid ${usuario?.aei_aprobado === true ? '#d1fae5' : '#e0e0e0'}`, borderRadius: 6, overflow: 'hidden', background: usuario?.aei_aprobado === true ? '#f0fdf4' : '#fff' }}>
                        <span style={{ padding: '10px 8px 10px 12px', fontSize: 13, color: usuario?.aei_aprobado === true ? '#065f46' : '#555', background: usuario?.aei_aprobado === true ? '#d1fae5' : '#f5f5f5', borderRight: `1px solid ${usuario?.aei_aprobado === true ? '#a7f3d0' : '#e0e0e0'}`, userSelect: 'none', fontWeight: 700 }}>AEI-</span>
                        <input
                          value={perfilAei}
                          readOnly={usuario?.aei_aprobado === true}
                          onChange={e => setPerfilAei(e.target.value.replace(/\D/g, ''))}
                          inputMode="numeric"
                          placeholder='0000'
                          style={{ flex: 1, border: 'none', padding: '10px 12px', fontSize: 13, outline: 'none', background: 'transparent', color: usuario?.aei_aprobado === true ? '#065f46' : '#111', cursor: usuario?.aei_aprobado === true ? 'not-allowed' : 'text', fontWeight: usuario?.aei_aprobado === true ? 700 : 400 }}
                          onFocus={e => { if (usuario?.aei_aprobado !== true) { const p = e.target.closest('div[data-aei]') as HTMLElement; if (p) p.style.borderColor='#006D77' } }}
                          onBlur={e => { if (usuario?.aei_aprobado !== true) { const p = e.target.closest('div[data-aei]') as HTMLElement; if (p) p.style.borderColor='#e0e0e0' } }}
                        />
                      </div>
